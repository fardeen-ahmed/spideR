//Spider script for enumerating domains. Some codes are taken from subfinder, and is added with comments snippet as "Taken from subfinder". Subfinder can be found here script found here :- https://github.com/projectdiscovery/subfinder
package main
import (
	"encoding/json"
	"flag"
...
func writeTxtFile(domain string) {
	...
	str += fmt.Sprintln("=================================================================================")//added a look here, made by me.
	for sd, attributes := range Subdomains {
		sno += 1
		numIpAddrs := len(attributes.ip)
		ipAddrs := ""
		if numIpAddrs > 2 && domain == "" { // for displaying 2 IP on terminal to save real estate
...
func subDomainsFromSearchEngines(domain string) {
	for searchEngineName, attributes := range searchEngines {
		...
		//Taken from subfinder till line 262
		if len(subdomains) != 0 {
...
/* Some of the parameters :
   ...
   returns: a map[string]int of subdomains along with number of occurance
*/
...
// does a HTTP GET and returns the HTML body for that URL. Added from subfinder
...
/* Parse command line flage and initiaize the global flag variables. Made by me */
func initFlags() string {

	flag.Usage = func() {
		fmt.Fprintln(os.Stderr, "---------------------------------------------------------")
		fmt.Fprintln(os.Stderr, "Usage:   $ spider -d <domain-name> [-f <format(s)>] [-l 1]")
		fmt.Fprintln(os.Stderr, "Example: $ spider -d example.org")
		fmt.Fprintln(os.Stderr, "---------------------------------------------------------\nFlags:")
		flag.PrintDefaults()
	}
	...
	flag.Parse()
	//Made by me.
	// Use domain name is of a form of a TLD eg. "example.com"
